use crate::mock::*;
use crate::{CumulativeBurn, CumulativeMint, Elasticity, OperatorBalance};
...
fn signed_origin_rejected_on_privileged_calls() {
    new_test_ext().execute_with(|| {
        ...
        .is_err());
        assert!(crate::Pallet::<Test>::mint_to_operator(
            RuntimeOrigin::signed(1),
            42,
            10,
        )
        .is_err());
        assert!(
            crate::Pallet::<Test>::set_elasticity(RuntimeOrigin::signed(1), 1).is_err()
        );
    });
}
